...
class AFMDataset(Dataset):
    ...
    def __init__(self, triplets_df, user_features, book_features, 
        ...
        if fit_encoders:
            ...
            self.user_num_cols = ['user_rating_count', 'user_avg_rating', 'user_rating_std']
            self.book_num_cols = ['book_rating_count', 'book_avg_rating', 'popularity_norm']
            
            # Фильтруем только существующие колонки
            self.user_num_cols = [col for col in self.user_num_cols if col in user_features.columns]
            self.book_num_cols = [col for col in self.book_num_cols if col in book_features.columns]
            ...
            self.user_num_cols = [col for col in user_features.columns if col.startswith('user_')]
            self.book_num_cols = [col for col in book_features.columns if col.startswith('book_')]
        
        print(f"   User numerical features: {self.user_num_cols}")
        print(f"   Book numerical features: {self.book_num_cols}")
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            'user_categorical': torch.tensor([], dtype=torch.long),
            'pos_categorical': torch.tensor([], dtype=torch.long),
            'neg_categorical': torch.tensor([], dtype=torch.long),
